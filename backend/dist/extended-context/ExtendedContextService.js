...
exports.ExtendedContextService = void 0;
const ContextWindowManager_1 = require("./manager/ContextWindowManager");
...
class ExtendedContextService {
    ...
    constructor(config) {
        ...
        this.windowManager = new ContextWindowManager_1.ContextWindowManager(this.config.anthropicApiKey, {
        ...
        this.compressor = new ContextCompressor_1.ContextCompressor(this.config.anthropicApiKey, this.config.model);
    }
    /**
     * Initialize service
     */
    ...
    async addMessage(conversationId, role, content, metadata) {
        ...
        const message = await this.windowManager.addMessage(conversationId, role, content, metadata);
        if (isNew) {
        ...
        if (this.config.enableSnapshots) {
            ...
            if (window && window.messages.length % this.config.snapshotInterval === 0) {
                await this.windowManager.createSnapshot(conversationId, `Auto-snapshot at ${window.messages.length} messages`);
            }
        }
        return message;
    }
     ...
     */
    async compress(conversationId, strategy) {
        if (strategy) {
     ...
     */
    async restoreSnapshot(conversationId, snapshotId) {
    ...
    async summarize(request) {
        ...
        const result = await this.compressor.summarizeMessages(messages);
        return result;
    }
    /**
     * Split conversation into chunks
     */
    splitIntoChunks(conversationId, options) {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (currentTokens + messageTokens > options.chunkSize &&
                ...
                if (options.overlapSize > 0) {
                    const overlapMessages = this.getOverlapMessages(currentChunk, options.overlapSize);
     ...
     */
    getOverlapMessages(messages, overlapTokens) {
        const overlap = [];
        let tokens = 0;
     ...
     */
    countTokens(text) {
        return TokenCounter_1.TokenCounter.countTokens(text);
    }
    /**
     * Estimate conversation tokens
     */
    estimateTokens(params) {
        return TokenCounter_1.TokenCounter.estimateConversationTokens(params);
    }
    /**
     * Check if conversation needs compression
     */
    needsCompression(conversationId) {
    ...
    getStats() {
        ...
        const averageTokens = windows.length > 0 ? totalTokens / windows.length : 0;
        const largestConversation = windows.reduce((max, w) => Math.max(max, w.totalTokens), 0);
        ...
        return {
            ...
            averageCompressionRatio: this.stats.totalCompressions > 0
...
exports.ExtendedContextService = ExtendedContextService;
//# sourceMappingURL=ExtendedContextService.js.map