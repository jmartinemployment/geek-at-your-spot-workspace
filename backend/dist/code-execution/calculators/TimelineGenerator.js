...
class TimelineGenerator {
    ...
    constructor(config) {
        if (config?.workHoursPerDay)
            this.workHoursPerDay = config.workHoursPerDay;
        if (config?.workDaysPerWeek)
            this.workDaysPerWeek = config.workDaysPerWeek;
        if (config?.bufferPercentage)
            this.bufferPercentage = config.bufferPercentage;
    }
    /**
     * Generate complete project timeline with dependencies
     */
    generate(input) {
        const { phases: inputPhases, teamSize = 2, workHoursPerDay = this.workHoursPerDay, workDaysPerWeek = this.workDaysPerWeek, buffer = this.bufferPercentage, } = input;
        ...
        for (const phaseInput of inputPhases) {
            const phase = this.generatePhase(phaseInput, currentWeek, teamSize, workHoursPerDay, workDaysPerWeek, buffer, dependencyGraph);
            phases.push(phase);
    ...
    calculateCriticalPath(graph) {
        ...
        const calculateEarliestStart = (node) => {
    ...
    identifyParallelTasks(graph) {
        ...
        for (const node of graph) {
            ...
            for (const otherNode of graph) {
                ...
                const hasCircularDep = node.dependencies.includes(otherNode.id) ||
     ...
     */
    generatePhase(phaseInput, startWeek, teamSize, workHoursPerDay, workDaysPerWeek, buffer, dependencyGraph) {
        const tasks = [];
        let currentDay = 0;
     ...
     */
    generateMilestones(phaseName, tasks) {
        const milestones = [];
        // Start milestone
     ...
     */
    generateWarnings(phases, criticalPath, dependencyGraph) {
        const warnings = [];
        ...
        if (bottlenecks.length > 0) {
            warnings.push(`${bottlenecks.length} potential bottleneck(s) detected: ${bottlenecks
                .map(n => n.name)
                .slice(0, 3)
                .join(', ')}`);
        }
        ...
        if (criticalPath.length > 10) {
            warnings.push(`Critical path is long (${criticalPath.length} tasks) - delays will cascade through project`);
        }
        // Check for tasks with no dependencies (possible missing dependencies)
        const independentTasks = dependencyGraph.filter(node => node.dependencies.length === 0 && node.dependents.length === 0);
        if (independentTasks.length > 5) {
            warnings.push(`${independentTasks.length} independent tasks - verify dependencies are correctly specified`);
        }
        return warnings;
    }
    /**
    ...
    optimize(input) {
        ...
        if (analysis.parallelizable.length > 0) {
            ...
            timeline.warnings.push(`${analysis.parallelizable.length} groups of tasks can be parallelized to reduce timeline`);
        }
        return timeline;
    }
    /**
