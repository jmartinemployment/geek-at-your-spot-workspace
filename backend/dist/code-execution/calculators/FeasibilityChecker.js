...
exports.FeasibilityChecker = void 0;
class FeasibilityChecker {
     ...
     * Check technology compatibility
     */
    checkTechnologyCompatibility(technologies) {
        const risks = [];
    ...
    hasKnownConflict(tech1, tech2) {
        ...
        return conflicts.some(([t1, t2]) => (tech1.includes(t1) && tech2.includes(t2)) ||
            (tech1.includes(t2) && tech2.includes(t1)));
    }
    /**
     * Check feature complexity
     */
    checkFeatureComplexity(features) {
        const risks = [];
     ...
     * Check project type feasibility
     */
    checkProjectType(projectType, features) {
        const risks = [];
        // Check if features are appropriate for project type
        if (projectType === 'website') {
            const mobileFeatures = features.filter(f => f.toLowerCase().includes('mobile') ||
     ...
     */
    generateTechnologyRecommendations(input) {
        const recommendations = [];
        // Check if missing essential technologies
        const hasDatabase = input.technologies.some(t => ['postgresql', 'mysql', 'mongodb', 'sqlite'].includes(t.toLowerCase()));
        ...
        const hasAuth = input.features.some(f => f.toLowerCase().includes('auth') || f.toLowerCase().includes('login'));
     ...
     */
    generateProcessRecommendations(risks) {
        const recommendations = [];
     ...
     */
    calculateFeasibilityScore(risks, input) {
        let score = 100;
     ...
     * Identify technical constraints
     */
    identifyTechnicalConstraints(input) {
        const constraints = [];
     ...
     */
    suggestAlternativeApproaches(input, risks) {
        const alternatives = [];
...
exports.FeasibilityChecker = FeasibilityChecker;
//# sourceMappingURL=FeasibilityChecker.js.map