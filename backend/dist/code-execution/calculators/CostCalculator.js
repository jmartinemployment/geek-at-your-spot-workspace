...
class CostCalculator {
     ...
     */
    calculate(input) {
        const { projectType, features, complexity = 'medium', teamSize = 2, hourlyRate = this.baseHourlyRate, } = input;
        ...
        for (const feature of features) {
            this.calculateFeatureCost(feature, hourlyRate, complexityMult, projectMult, breakdown, processedFeatures);
        }
        // Calculate totals
     ...
     */
    calculateFeatureCost(feature, hourlyRate, complexityMult, projectMult, breakdown, processedFeatures) {
        ...
        for (const dep of dependencies) {
            this.calculateFeatureCost(dep, hourlyRate, complexityMult, projectMult, breakdown, processedFeatures);
        }
     ...
     */
    estimateUnknownFeature(feature) {
        const name = feature.toLowerCase();
        if (name.includes('simple') || name.includes('basic'))
            return 8;
        if (name.includes('complex') || name.includes('advanced'))
            return 32;
        if (name.includes('integration'))
            return 20;
        if (name.includes('dashboard') || name.includes('panel'))
            return 24;
        if (name.includes('notification'))
            return 16;
        if (name.includes('authentication') || name.includes('auth'))
            return 16;
        // Default estimate
        return 16;
    }
     ...
     */
    generateTimeline(breakdown, teamSize) {
        const totalHours = breakdown.reduce((sum, item) => sum + item.hours, 0);
     ...
     */
    compareScenarios(baseInput, scenarios) {
