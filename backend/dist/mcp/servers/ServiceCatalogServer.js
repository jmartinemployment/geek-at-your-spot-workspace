...
exports.ServiceCatalogServer = void 0;
class ServiceCatalogServer {
    ...
    async searchServices(params, _context) {
        try {
            let results = [...this.serviceCatalog];
            if (params.query) {
                const query = params.query.toLowerCase();
                results = results.filter((service) => service.name.toLowerCase().includes(query) ||
    ...
    async getServiceDetails(params, _context) {
        try {
    ...
    async recommendServices(params, _context) {
        try {
            ...
            const scoredServices = this.serviceCatalog.map((service) => {
                ...
                keywords.forEach((keyword) => {
                    ...
                    if (service.description.toLowerCase().includes(keyword))
                        score += 2;
                    if (service.features.some((f) => f.toLowerCase().includes(keyword)))
                        score += 1;
                });
                if (budget) {
    ...
    async calculateServicePackage(params, _context) {
        try {
            ...
            const subtotal = services.reduce((sum, service) => sum + (service.price_range.min + service.price_range.max) / 2, 0);
            let discount = 0;
            ...
            const estimatedDurationWeeks = services.reduce((sum, service) => sum + service.duration_weeks, 0);
            const result = {
                services,
                subtotal,
                discount,
    ...
    generateReasons(service, params) {
        const reasons = [];
...
exports.ServiceCatalogServer = ServiceCatalogServer;
//# sourceMappingURL=ServiceCatalogServer.js.map