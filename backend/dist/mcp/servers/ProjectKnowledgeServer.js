...
exports.ProjectKnowledgeServer = void 0;
class ProjectKnowledgeServer {
     ...
     */
    async searchPastProjects(params, context) {
        try {
            ...
            if (params.budget_range) {
                where.cost = {};
                if (params.budget_range.min)
                    where.cost.gte = params.budget_range.min;
                if (params.budget_range.max)
                    where.cost.lte = params.budget_range.max;
            }
     ...
     */
    async getProjectStatistics(params, context) {
        try {
            ...
            const statistics = {
                ...
                by_type: Object.fromEntries(projectsByType.map((item) => [item.type, item._count])),
                average_cost_by_type: Object.fromEntries(avgCostByType.map((item) => [item.type, item._avg.cost || 0])),
                average_duration_by_type: Object.fromEntries(avgCostByType.map((item) => [item.type, item._avg.durationWeeks || 0])),
                average_satisfaction: avgCostByType.reduce((sum, item) => sum + (item._avg.clientSatisfaction || 0), 0) /
     ...
     */
    async getTechnologyRecommendations(params, context) {
        try {
            ...
            const recommendations = Object.entries(techAnalysis)
                .map(([tech, data]) => {
                ...
                const reasons = [];
                if (successRate > 4.5)
                    reasons.push('Very high client satisfaction');
                if (count > 5)
                    reasons.push(`Used in ${count} successful projects`);
                if (budget && avgCost <= budget)
                    reasons.push('Fits within budget');
                if (timeline_weeks && avgDuration <= timeline_weeks)
...
exports.ProjectKnowledgeServer = ProjectKnowledgeServer;
//# sourceMappingURL=ProjectKnowledgeServer.js.map