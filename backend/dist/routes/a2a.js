...
router.post('/execute', async (req, res) => {
    try {
        const a2aService = req.app.locals.a2aService;
 ...
 */
router.get('/conversations/:conversationId/summary', (req, res) => {
    try {
        const a2aService = req.app.locals.a2aService;
        if (!a2aService) {
            return res.status(400).json({
 ...
 */
router.get('/conversations/recent', (req, res) => {
    try {
        const a2aService = req.app.locals.a2aService;
        if (!a2aService) {
            return res.status(400).json({
...
router.get('/agents/role/:role', (req, res) => {
    try {
        ...
        const agents = a2aService.getAgentsByRole(role);
        return res.status(200).json({
            role,
            agents,
            total: agents.length,
 ...
 */
router.post('/agents/:agentId/enable', (req, res) => {
    try {
        const a2aService = req.app.locals.a2aService;
        if (!a2aService) {
            return res.status(400).json({
