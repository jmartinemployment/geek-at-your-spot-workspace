...
export class GeekQuoteAiComponent implements OnInit {
  ...
  private async processMessage(content: string): Promise<void> {
    ...
    try {
      ...
      // Open dialog if not already open, otherwise just update title
      if (!this.dialogService.isOpen()) {
        ...
        // Update title for existing open dialog
        this.dialogService.title.set(dialogTitle);
      }
